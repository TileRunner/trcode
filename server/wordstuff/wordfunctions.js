  ...
  for (var i = 0; i < wlc.length; i++) {
    for (var j = 0; j < 26; j++) {
      if (wlc[i] !== alphabet[j]) {
        ...
        if (swaps.indexOf(checkword) < 0 && wordssamelength[checkword] > -1) {
...
function getDropWords(word, wordsonelesslength) {
  ...
  for (var i = 0; i < wlc.length; i++) {
    ...
    let checkword = testword.join('');
    if (wordsonelesslength[checkword]) {
 ...
 * @param {object[]} anagramsByLength An array of word lists where the index is the word length up to 8.
 * @param {object[]} alphagramsByLength An array of alphagrams lists with anagrams where the index is the word length up to 8.
 ...
 */
function createTransmogrifyPuzzle(anagramsByLength=[], alphagramsByLength=[], minMoves=2) {
  let fail = '';
